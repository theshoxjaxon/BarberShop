  ...
  return (
    ...
    >
            ...
            <div className="gallery-col space-y-4 lg:space-y-5 will-change-transform" data-speed="-80">
              {col1Images.map((img, i) => (
                <div key={i} className="gallery-img-wrap overflow-hidden will-change-transform" data-offset={i === 0 ? "60" : "120"}>
                  ...
                  <p className="museo-label text-[#C9A24D]/50 mt-3 text-[10px]">{img.label}</p>
                </div>
              ))}
            </div>
      ...
      {aboutConfig.stats.length > 0 && (
        <div ref={statsRef} className="relative z-10 max-w-7xl mx-auto px-8 lg:px-16 py-24 bg-[#0A0A0A]">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-white/10 pt-12">
            {aboutConfig.stats.map((stat, i) => (
              <div key={i} className="stat-item">
                <p className="museo-headline text-[#C9A24D] text-4xl md:text-5xl mb-2">{stat.value}</p>
      ...
      {aboutConfig.bottomText && (
        <div className="relative z-10 max-w-7xl mx-auto px-8 lg:px-16 pb-32 bg-[#0A0A0A]">
